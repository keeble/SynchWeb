<template>
  <div class="content">
    ...
    <div class="form">
      ...
      <validation-provider slim name="Thickness" :rules="getValidationRules('thickness')" v-slot="{ errors }">
        <base-input-text
          label="Thickness"
          v-model="inputValue[sampleIndex].THICKNESS"
          :errorMessage="errors[0]"/>
      </validation-provider>

      <validation-provider slim :rules="getValidationRules('background')" v-slot="{ errors }">
        <base-input-text
          label="Background 1"
          v-model="inputValue[sampleIndex].BACKGROUND1"
          :errorMessage="errors[0]"/>
      </validation-provider>
      ...
      <validation-provider slim :rules="getValidationRules('background')" v-slot="{ errors }">
        <base-input-text
          label="Background 2"
          v-model="inputValue[sampleIndex].BACKGROUND2"
          :errorMessage="errors[0]"/>
      </validation-provider>

      <validation-provider slim :rules="getValidationRules('qrange')" v-slot="{ errors }">
        <base-input-select v-model="inputValue[sampleIndex].QRANGE"
          label="Q Range"
          :options="qRangeOptions"
          optionTextKey="TEXT"
          optionValueKey="ID"
          :errorMessage="errors[0]"/>
      </validation-provider>
