...
import { SampleTableNewMap, SampleTableViewMap, SingleSampleMap } from 'modules/types/saxs/samples/experiments/sample-table-map'
...
export default {
  ...
  computed: {
    sampleComponent: function() {
      ...
      let component = sampleTableMap[experimentType] ? sampleTableMap[experimentType] : sampleTableMap['default']

      if (this.containerType.CAPACITY > 25) {
        console.log("Should use single sample view")
        component = SingleSampleMap[experimentType] ? SingleSampleMap[experimentType] : SingleSampleMap['default']
      }
