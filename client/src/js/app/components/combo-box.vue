...
The v-closable takes an object as argument with properties:
- excludeElements: An array or refs that should not trigger the closing of the combobox
  ...
  >
    ...
    <div class="select-items select-hide" :class="{[`select-${inputIndex}`]: true}" :ref="`select-items-${inputIndex}`">
      <div class="items-list">
        ...
        <div class="tw-w-full tw-flex tw-flex-row tw-justify-center custom-add" v-if="canCreateNew && filteredOptions.length < 1 && searchText.length > 0">
